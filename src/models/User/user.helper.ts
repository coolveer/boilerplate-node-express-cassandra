...
import {decodeAddress,ed25519PairFromSeed,cryptoWaitReady,mnemonicGenerate,encodeAddress,Keyring,mnemonicToMiniSecret,util_crypto_encodeAddress,mnemonicValidate,ApiPromise,WsProvider,ContractPromise,Abi,DecodedEvent,CryptoJS} from './user.common'
import {responseHelper} from '../../helper/responses'
const {STATUS,RES_MSG} = require('../../helper/responses/const.responses')
var Person = connect.loadSchema('Person', require('./user.model'));
...
class UserHelper {
    ...
    async userSignUp(res:Response,payload:{
        name: string;
        email: string,
        password: string,
        wallet:string,
        ...
        payload.mnemonics = mnemonic;
            ...
            var user:any = new Person(payload);
